...
def test_include_part(s3_upload: S3Upload) -> None:
    glacier_transfer_part = GlacierTransferPart(
        ...
        glacier_object_id="test_id",
        part_number=1,
        e_tag="test-etag",
        checksum_sha_256="test-checksum",
    )
    s3_upload.include_part(glacier_transfer_part)
    assert s3_upload.parts == [
        {
            "PartNumber": int(glacier_transfer_part.part_number),
            "ETag": glacier_transfer_part.e_tag,
            "ChecksumSHA256": glacier_transfer_part.checksum_sha_256,
        }
    ]
